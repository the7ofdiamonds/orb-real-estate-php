<?php
/**
 * @package ORBRealEstate
 */

class ORBRealEstatePostType
{
    public function __construct() {
        add_action('init', [$this, 'real_estate_custom_post_type']);
        add_action( 'admin_init', [$this, 'add_real_estate_meta_boxes'] );
        add_action( 'save_post', [$this, 'save_post_real_estate'] );
        add_action( 'rest_api_init', [$this, 'add_address_to_rest'] );
        add_action('init', [$this, 'real_estate_custom_taxonomies']);
        add_filter( 'archive_template', array( $this, 'get_real_estate_archive_template' ) );
        add_filter( 'single_template', array( $this, 'get_real_estate_single_template' ), 10, 1 );
    }

    public function real_estate_custom_post_type (){
        $labels = array(
            'name' => 'Real Estate',
        ...
        $args = array(
            ...
            'rewrite' => array(
                'with_front' => true,
                'slug'       => 'services/invest/real-estate'
            ),
    ...
    function add_address_to_rest() {
        register_rest_field( 'real-estate', 'home_type', array(
            'get_callback' => [$this, 'get_prop_home_type'])
        );

        register_rest_field( 'real-estate', 'address', array(
            'get_callback' => [$this, 'get_prop_address'])
        );

        register_rest_field( 'real-estate', 'bedrooms', array(
            'get_callback' => [$this, 'get_prop_bedrooms'])
        );

        register_rest_field( 'real-estate', 'bathrooms', array(
            'get_callback' => [$this, 'get_prop_bathrooms'])
        );

        register_rest_field( 'real-estate', 'sqft', array(
            'get_callback' => [$this, 'get_prop_sqft'])
        );

        register_rest_field( 'real-estate', 'lot_size', array(
            'get_callback' => [$this, 'get_prop_lot'])
        );

        register_rest_field( 'real-estate', 'price', array(
            'get_callback' => [$this, 'get_prop_price'])
        );

        register_rest_field( 'real-estate', 'year_built', array(
            'get_callback' => [$this, 'get_prop_year_built'])
        );

        register_rest_field( 'real-estate', 'heating', array(
            'get_callback' => [$this, 'get_prop_heating'])
        );

        register_rest_field( 'real-estate', 'cooling', array(
            'get_callback' => [$this, 'get_prop_cooling'])
        );

        register_rest_field( 'real-estate', 'garage', array(
            'get_callback' => [$this, 'get_prop_garage'])
        );

        register_rest_field( 'real-estate', 'featured_media_url', array(
                'get_callback'    => [$this, 'get_prop_featured_media'])
        );

        register_rest_field( 'real-estate', 'driveway', array(
            'get_callback' => [$this, 'get_prop_driveway'])
        );

        register_rest_field( 'real-estate', 'story', array(
            'get_callback' => [$this, 'get_prop_story'])
        );

        register_rest_field( 'real-estate', 'appliances', array(
            'get_callback' => [$this, 'get_prop_appliances'])
        );

        register_rest_field( 'real-estate', 'flooring', array(
            'get_callback' => [$this, 'get_prop_flooring'])
        );

        register_rest_field( 'real-estate', 'construction_materials', array(
            'get_callback' => [$this, 'get_prop_materials'])
        );

        register_rest_field( 'real-estate', 'foundation', array(
            'get_callback' => [$this, 'get_prop_foundation'])
        );

        register_rest_field( 'real-estate', 'parcel_number', array(
            'get_callback' => [$this, 'get_prop_parcel'])
        );

        register_rest_field( 'real-estate', 'gallery', array(
...
if ( class_exists( 'ORBRealEstatePostType' )) {
    $orbRealEstatePostType = new ORBRealEstatePostType();
}

register_activation_hook( __FILE__, array( $orbRealEstatePostType, 'activate' ));
register_deactivation_hook( __FILE__, array( $orbRealEstatePostType, 'deactivate' ));